...
router.use(session({secret: 'adminSecret1', resave: false, saveUninitialized: true}));
...
router.post('/login', (req, res) => {
    ...
    User.findOne({ email: req.body.email }, (err, user) => {
      ...
      if (err) return res.status(500).send('Something shows error on the server.');
      let htmlMsg
      if (!user) { 
        htmlMsg = encodeURIComponent('Email or password is not found please can you try again');
        res.redirect('/?invalid=' + htmlMsg);
      }
      else{
        const passwordIsValid = bcrypt.compareSync(req.body.password, user.password);
        if (!passwordIsValid) {
          return res.status(401).send('Email or password is not found please can you try again');
        }
